...
def grad_norm_from_list(par_list):
    ...
    if len(parameters) == 0:
        ...
        total_norm = torch.norm(
            torch.stack(
                [torch.linalg.norm(p.grad.detach()).to(device) for p in parameters]
            ),
            2.0,
        ).item()
    return total_norm
...
def optimize_torch_obj_fun(
    obj_fun_,
    unParIn,
    optimizer,
    max_opt_iter=1000,
    rel_improv_tol=5e-8,
    no_improv_max_count=10,
    min_opt_iter=50,
    bandwidth=10,
    small_grad_th=1e-3,
    tb_folder="tb_logs",
    tb_log_flag=True,
    hparams_dict_in=None,
    run_name="",
    log_interval=200,
    disable_logging=False,
    clip_grad_norm_to=None,
    clip_grad_val_to=1e3,
):
    ...
    # optimizers = {
    #     "SGD": torch.optim.SGD(unPar, lr=lr, nesterov=False),
    ...
    hparams_dict = {
        # "optimizer": opt_n,
        # "lr": lr,
        "n_par": sum((p.numel() for p in unPar)),
    }

    opt_info_str = run_name + "".join(
        [f"{key}_{value}_" for key, value in hparams_dict.items()]
    )
    ...
    for n_iter in range(max_opt_iter):
        ...
        try:
            ...
            if (n_iter % log_interval) == 0:
                logger.info(
                    f" iter {n_iter}, g_norm {'{:.3e}'.format(grad_norm)}, g_max {'{:.3e}'.format(grad_max)}, roll rel impr { '{:.3e}'.format( roll_rel_im)},  loss {'{:.5e}'.format(loss.item())}"
                )
        except:
            raise Exception(f"Error at iter {n_iter}, with parameters {unPar}")

    opt_metrics = {
        "actual_n_opt_iter": n_iter,
        "final_grad_norm": grad_norm,
        "final_grad_max": grad_max,
        "final_roll_improv": roll_rel_im,
    ...
    logger.info(
        f"OPT TERMINATED {hparams_dict}. FINAL LOSS={final_loss.item()} EXIT FLAGS:  no_improv_flag = {no_improv_flag} ,  small_grad_flag = {small_grad_flag} , nan_flag = {nan_flag}, {opt_metrics}"
    )
